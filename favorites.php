...
if (!isset($_SESSION['user_id'])) {
    die("Error: You have to be authorised to view this page.");
}
    ...
    <?php else: ?>
        <?php foreach ($favorites as $post): ?>
            <div class="post">
                ...
                <?php endif; ?>
                <p style="margin-top: 10px"><?= htmlspecialchars($post['content']) ?></p>
            ...
            <div class="post">
                <?php if (!empty($post['image'])): ?>
                    <img src="<?= htmlspecialchars($post['image']); ?>"
                         alt="Post image link is corrupted. Check post source."
                         style="max-width: 100%; max-height: 400px; object-fit: cover; border-radius: 10px; display: block; margin: 0 auto;">
                <?php else: ?>
                    <p style="color: gray; font-style: italic;">This post doesn’t have an image attached.</p>
                <?php endif; ?>
